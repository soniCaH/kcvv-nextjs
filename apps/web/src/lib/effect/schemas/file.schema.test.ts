...
describe("File Schema", () => {
  describe("FileUri", () => {
    it("should parse valid file URI", () => {
      ...
      expect(result.url).toBe(
        "https://api.kcvvelewijt.be/sites/default/files/2024-01/hero.jpg",
      );
    });
  ...
  describe("FileAttributes", () => {
    it("should parse valid file attributes", () => {
      const validAttributes = {
        ...
        uri: {
          url: "https://api.kcvvelewijt.be/sites/default/files/article-hero.jpg",
        },
        filemime: "image/jpeg",
        filesize: 245678,
        status: true,
      ...
      expect(result.uri.url).toBe(
        "https://api.kcvvelewijt.be/sites/default/files/article-hero.jpg",
      );
    ...
    it("should reject attributes without uri", () => {
      ...
      expect(() =>
        S.decodeUnknownSync(FileAttributes)(invalidAttributes),
      ).toThrow();
    });
  ...
  describe("File", () => {
    it("should parse complete file entity", () => {
      const validFile = {
        ...
        attributes: {
          ...
          uri: {
            url: "https://api.kcvvelewijt.be/sites/default/files/team-photo.jpg",
          },
          filemime: "image/jpeg",
          filesize: 156789,
          status: true,
    ...
    it("should parse minimal file entity", () => {
      ...
      expect(result.type).toBe("file--file");
      expect(result.attributes.uri.url).toBe(
        "/sites/default/files/minimal.jpg",
      );
    });
  ...
  describe("Real-world JSON:API example", () => {
    it("should parse file from Drupal JSON:API response", () => {
      ...
      expect(result.attributes.uri.url).toBe(
        "/sites/default/files/2024-01/eerste-elftal-2024.jpg",
      );
