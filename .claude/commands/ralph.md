...
After quality checks pass, run **both** review skills on the branch diff (`git diff origin/main...HEAD`) before pushing. This is a **required gate on every Ralph issue**, not optional. Treat it as the last automated read the branch gets: CodeRabbitAI is one free review an hour on this account and is routinely rate-limited, so it is a bonus when it lands, never the backstop this gate is allowed to lean on.
