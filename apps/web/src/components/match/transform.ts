...
function assertNever(value: never): never {
  throw new Error(`Unhandled ScheduleRow kind: ${String(value)}`);
}
 ...
 *
 * `kind` is resolved once into its own variable and switched over explicitly
 * (#2802 review) rather than three cascading `if`s ending in a bare
 * fallthrough `return` — `apps/web/CLAUDE.md`'s exhaustiveness rule applies
 * to picking a union member just as much as to narrowing one: the `default:
 * assertNever(kind)` below is unreachable today, but it is what makes a
 * fourth `kind` a compile error here instead of a silently mis-shaped object.
 *
...
export function transformMatchToSchedule(match: Match): ScheduleRow {
  const kind: ScheduleRow["kind"] = match.is_placeholder
    ? "reservation"
    : isReducedMatchRow({
          isPlaceholder: false,
