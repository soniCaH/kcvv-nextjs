 ...
 *
 * @returns The rendered React nodes for the organogram UI
 */
export function OrganogramClient() {
  const [selectedMember, setSelectedMember] = useState<OrgChartNode | null>(null)
  const [isModalOpen, setIsModalOpen] = useState(false)
  const [departmentFilter, setDepartmentFilter] = useState<'all' | 'hoofdbestuur' | 'jeugdbestuur'>('all')

  // Filter data based on selected department
  const filteredData = departmentFilter === 'all'
    ? clubStructure
    : clubStructure.filter(node =>
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
        <div className="bg-white rounded-lg shadow-sm p-6 border-t-4 border-green-main">
          <div className="flex items-center gap-3 mb-2">
            <svg className="w-6 h-6 text-green-main" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 15l-2 5L9 9l11 4-5 2zm0 0l5 5M7.188 2.239l.777 2.897M5.136 7.965l-2.898-.777M13.95 4.05l-2.122 2.122m-5.657 5.656l-2.12 2.122" />
            </svg>
            <h3 className="font-bold text-gray-blue">Klik om uit/in te klappen</h3>
          </div>
          <p className="text-sm text-gray-dark">
            Klik op een kaartje om de onderliggende functies te zien of te verbergen. Hele kaart is klikbaar!
          </p>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-sm p-6 border-t-4 border-green-main">
          <div className="flex items-center gap-3 mb-2">
            <svg className="w-6 h-6 text-green-main" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-2-2m0 0l-2.5-2.5M19 19l-2.5-2.5m0 0L11 11m5 5l-2.5-2.5M11 11l-2-2m2 2l-2.5-2.5M11 11V7.5M11 11H7.5" />
            </svg>
            <h3 className="font-bold text-gray-blue">Gebruik de knoppen</h3>
          </div>
          <p className="text-sm text-gray-dark">
            Gebruik de + en − knoppen om in/uit te zoomen. Sleep met de muis om te pannen.
          </p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg shadow-sm p-6">
        <h2 className="text-xl font-bold text-gray-blue mb-4" style={{ fontFamily: 'quasimoda, acumin-pro, Montserrat, sans-serif' }}>
          Over onze organisatie
        </h2>
        <div className="space-y-3 text-gray-dark">
          <p>
            KCVV Elewijt heeft een uitgebreide organisatiestructuur met zowel een <strong>Hoofdbestuur</strong> voor
            de algemene clubwerking als een <strong>Jeugdbestuur</strong> dat zich specifiek focust op de ontwikkeling
            van onze jeugdspelers.
          </p>
          <p>
            Ons bestuur werkt nauw samen met vrijwilligers, trainers, en ouders om een professionele en gezellige
            voetbalomgeving te creëren voor spelers van alle leeftijden.
          </p>
          <p className="text-sm text-gray-medium pt-2 border-t border-gray-light">
            <strong>Wil je contact opnemen?</strong> Klik op een bestuurslid in het organogram om de contactgegevens te bekijken,
            of stuur een e-mail naar <a href="mailto:info@kcvvelewijt.be" className="text-green-main hover:text-green-hover hover:underline">info@kcvvelewijt.be</a>.
          </p>
        </div>
      </div>
