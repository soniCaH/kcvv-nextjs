    ...
    <div className={cn('', className)}>
      ...
      {filteredSponsors.length === 0 ? (
        <div className="text-center py-12">
          ...
          <h3 className="text-xl font-bold text-kcvv-gray-blue mb-2">Geen sponsors gevonden</h3>
          <p className="text-gray-600">Probeer een andere zoekopdracht of filter.</p>
        </div>
      ) : (
        <>
          <SponsorsTier tier="gold" title="Gouden Sponsors" sponsors={filteredByTier.gold} />
          {filteredByTier.gold.length > 0 && filteredByTier.silver.length > 0 && <TierDivider />}
          <SponsorsTier tier="silver" title="Zilveren Sponsors" sponsors={filteredByTier.silver} />
          {filteredByTier.silver.length > 0 && filteredByTier.bronze.length > 0 && <TierDivider />}
          <SponsorsTier tier="bronze" title="Bronzen Sponsors" sponsors={filteredByTier.bronze} />
        </>
      )}
    </div>
  )
