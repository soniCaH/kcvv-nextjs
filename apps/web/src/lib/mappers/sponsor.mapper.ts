 ...
 */
export function mapSponsorToComponentSponsor(
  drupalSponsor: DrupalSponsor,
): Sponsor {
  // Extract logo URL from resolved media relationship
  const logoData = drupalSponsor.relationships.field_media_image?.data;
  const logoUrl =
    logoData && "uri" in logoData && logoData.uri?.url
      ? logoData.uri.url
      : "/images/placeholder-sponsor.png";
 ...
 */
export function mapSponsorsToComponentSponsors(
  drupalSponsors: readonly DrupalSponsor[],
): Sponsor[] {
