 ...
 * Angels Page
 *
 * Fetches the Angels team from Drupal and delegates rendering to BestuurPage.
 */
...
const ANGELS_SLUG = "angels";

async function fetchAngelsOrNotFound(): Promise<TeamWithRoster> {
  try {
    return await runPromise(
      Effect.gen(function* () {
        const drupal = yield* DrupalService;
        return yield* drupal.getTeamWithRoster(ANGELS_SLUG);
      }),
    );
  } catch (error) {
...
export async function generateMetadata(): Promise<Metadata> {
  try {
    const { team, teamImageUrl } = await fetchAngelsOrNotFound();
    const title = team.attributes.title;
    const tagline = getTeamTagline(team);
    const description = tagline
      ? `${title} — ${tagline}`
      : `De Angels van KCVV Elewijt`;
    ...
    return { title: "Angels | KCVV Elewijt" };
  }
}

export default async function AngelsPageRoute() {
  const { team, staff, players, teamImageUrl } = await fetchAngelsOrNotFound();
