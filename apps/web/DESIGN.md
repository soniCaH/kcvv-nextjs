...
- **Don't** draw a new divider in green. Rendered and rejected (C2): at divider density a run of green hairlines reads as a green section, which the Rare Green Rule forbids. The convention already has two shipped exceptions (`SiteFooter`, `HulpFinder`) rather than a clean record — don't add a third.
