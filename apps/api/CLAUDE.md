...
- **A gate token is pacing, not observability — incident alerting still does not cover the scheduled paths.** `gate.reportOutcome` (the call that opens/closes a PSD incident and fires the Slack ping from #2329) is invoked from the `TypedKvCache` refresh path in `cache/kv-cache.ts` and nowhere else. The nightly sync touches `KvCacheService` only for `increment()`, so it never reaches one. **A week of 02:00 sync failures against a 429-ing PSD produces a `console.error` inside `ctx.waitUntil` and nothing else** — no incident, no Slack, no drift nudge (drift is a cache-key concept and the sync writes no cached value). Do not read the pacing rule above as covering this. Closing the gap means deciding whether a sync failure should open the same incident the read path opens, which is a behaviour question, not a wiring one — a sync that fails while the site serves fine is not the same event as PSD being down for visitors.
