...
export const Sponsors = ({
  ...
  const descriptionColorClass = variant === 'dark' ? 'text-white/80' : 'text-gray-600'

  return (
    <section className={cn('py-8', className)}>
      {/* Section Header */}
      <div className="mb-6">
        <h3 className={cn('text-xl font-bold mb-2', textColorClass)}>{title}</h3>
        {description && (
          <p className={cn('text-sm opacity-80', descriptionColorClass)}>{description}</p>
        )}
      </div>
      ...
      {showViewAll && (
        <Link
          ...
          className={cn(
            ...
            variant === 'dark' ? 'text-kcvv-green-bright' : 'text-kcvv-green-dark'
          )}
        >
          Alle sponsors &raquo;
        </Link>
