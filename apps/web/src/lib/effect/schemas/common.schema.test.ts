...
describe("Common Schema - JSON:API Types", () => {
  describe("JsonApiVersion", () => {
    ...
    it("should reject missing version field", () => {
      ...
      expect(() =>
        S.decodeUnknownSync(JsonApiVersion)(invalidVersion),
      ).toThrow();
    });
  });
  ...
  describe("JsonApiLink", () => {
    it("should parse valid JSON:API link", () => {
      ...
      expect(result.href).toBe(
        "https://api.kcvvelewijt.be/jsonapi/node/article",
      );
      expect(result.meta).toBeUndefined();
    });
  ...
  describe("JsonApiLinks", () => {
    it("should parse complete pagination links", () => {
      const validLinks = {
        self: {
          href: "https://api.kcvvelewijt.be/jsonapi/node/article?page[offset]=20",
        },
        next: {
          href: "https://api.kcvvelewijt.be/jsonapi/node/article?page[offset]=40",
        },
        prev: {
          href: "https://api.kcvvelewijt.be/jsonapi/node/article?page[offset]=0",
        },
        first: {
          href: "https://api.kcvvelewijt.be/jsonapi/node/article?page[offset]=0",
        },
        last: {
          href: "https://api.kcvvelewijt.be/jsonapi/node/article?page[offset]=200",
        },
      };
...
describe("Common Schema - Drupal Structures", () => {
  describe("DrupalImage", () => {
    it("should parse complete image data", () => {
      const validImage = {
        uri: {
          url: "https://api.kcvvelewijt.be/sites/default/files/image.jpg",
        },
        alt: "Test Image",
        title: "Test Title",
        width: 1920,
      ...
      expect(result.uri.url).toBe(
        "https://api.kcvvelewijt.be/sites/default/files/image.jpg",
      );
