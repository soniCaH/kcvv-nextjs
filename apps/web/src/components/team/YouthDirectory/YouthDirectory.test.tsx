...
describe("YouthDirectory", () => {
  ...
  describe("the section heading (#2641)", () => {
    it("heads and names the section with what the route passed", () => {
      ...
      render(
        <YouthDirectory heading="Alle andere ploegen" divisions={divisions} />,
      );
  ...
  describe("the division sub-line (#2641)", () => {
    ...
    it("omits a reeks that only repeats the caption", () => {
      render(
        <YouthDirectory
          heading={HEADING}
          divisions={[
            {
              label: "Onderbouw",
              range: "U6–U11",
              teams: [team("U9", null, { divisionFull: "u9" })],
            },
          ]}
        />,
      );
      expect(paragraphsIn(screen.getByTestId("youth-team-card"))).toBe(1);
    });
  });
  ...
  describe("the Reserven group", () => {
    ...
    it("heads the group with where the side sits, not with an age band", () => {
      render(<YouthDirectory heading={HEADING} divisions={reserven} />);
    ...
    it("renders a rangeless group's heading bare (#2414)", () => {
      ...
      render(
        <YouthDirectory
          heading={HEADING}
          divisions={[{ label: "Reserven", teams: [reservenTeam()] }]}
        />,
      );
