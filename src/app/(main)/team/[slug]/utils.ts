...
/**
 * Calculate form string (e.g., "WWDLW") from recent matches
 *
 * @param matches - Array of matches (already sorted by date)
 * @param teamId - The team ID to calculate form for
 * @param count - Number of recent matches to include (default 5)
 * @returns Form string like "WWDLW" (W=win, D=draw, L=loss)
 */
export function calculateFormFromMatches(
  matches: readonly Match[],
  teamId: number,
  count: number = 5,
): string | undefined {
  // Filter to finished matches with scores, sorted by date descending (most recent first)
  const finishedMatches = [...matches]
    .filter(
      (m) =>
        typeof m.home_team.score === "number" &&
        typeof m.away_team.score === "number",
    )
  ...
  // Calculate result for each match
  const form = finishedMatches
    .map((match) => {
      ...
      const oppScore = isHome ? match.away_team.score! : match.home_team.score!;
...
/**
 * Inject calculated form into standings for a specific team
 *
 * @param standings - Array of standings entries
 * @param teamId - The team ID to inject form for
 * @param form - The calculated form string
 * @returns New array with form injected for the specified team
 */
export function injectFormIntoStandings(
  standings: StandingsEntry[],
  teamId: number,
  form: string | undefined,
): StandingsEntry[] {
  if (!form) return standings;

  return standings.map((entry) =>
    entry.teamId === teamId ? { ...entry, form } : entry,
  );
}
