...
3. **Greyscale-by-default → full colour on hover/focus is locked** for every logo site-wide
   ...
   in `<SponsorsBlock>`.
   **Superseded for `/sponsors` itself** by [#2511](https://github.com/soniCaH/www.kcvvelewijt.be/issues/2511)/[#2655](https://github.com/soniCaH/www.kcvvelewijt.be/issues/2655): "site-wide" no longer holds — `/sponsors` now renders every logo in colour at rest, at every tier. `<SponsorsBlock>` (homepage + team pages) is unaffected.
