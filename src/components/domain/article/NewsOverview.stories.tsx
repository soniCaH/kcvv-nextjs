  ...
  render: () => (
    <div className="w-full max-w-inner-lg mx-auto px-3 lg:px-0 py-6">
      ...
      <section className="mb-6 uppercase">
        ...
        <CategoryFilters categories={mockCategories} />
      </section>
...
export const WithActiveFilter: Story = {
  render: () => (
    <div className="w-full max-w-inner-lg mx-auto px-3 lg:px-0 py-6">
      ...
      <main className="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-10 mb-6">
        {mockArticles
          .filter((article) => article.tags.some((tag) => tag.name === 'Jeugd'))
          .map((article, index) => (
            <ArticleCard key={index} {...article} />
          ))}
      </main>
...
export const EmptyState: Story = {
  render: () => (
    <div className="w-full max-w-inner-lg mx-auto px-3 lg:px-0 py-6">
      <section className="mb-6 uppercase">
        <h5 className="mb-2 text-sm font-semibold">Filter op categorie</h5>
        <CategoryFilters categories={mockCategories} activeCategory="transfers" />
      </section>

      <main className="mb-6 text-center py-12">
        <p className="text-gray-500 text-lg">Geen artikelen gevonden voor deze categorie.</p>
      </main>
    </div>
  ),
}
