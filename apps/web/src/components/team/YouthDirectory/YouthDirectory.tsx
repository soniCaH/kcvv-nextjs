 ...
 * utility per tone lives here, beside its only consumer.
 */
const TONE_TEXT_CLASS: Record<YouthDivisionTone, string> = {
  ink: "text-ink",
...
export function YouthDirectory({
  ...
  return (
    ...
    >
      ...
      {groups.map((group) => {
        // Reserven's age is "A" — a senior code, not a bouw band — so it hits
        // `getYouthDivisionTone`'s null branch here directly, the same branch
        // any senior team hits. Deliberate, not an omission (#2615): the type
        // of `group.label` narrows to `YouthDivisionName` on every arm but
        // this one, so the ternary is exhaustive rather than a guess.
        const tone = getYouthDivisionTone(
          group.label === RESERVEN_LABEL ? null : group.label,
        );
        ...
        return (
          <div key={group.label} data-testid="youth-division">
            <h3
              className={cn(
                toneClass,
                "border-paper-edge mb-5 border-b pb-1.5 font-mono text-[11px] tracking-[0.1em] uppercase",
              )}
            >
              {group.label}
