...
describe('FeaturedArticles', () => {
  ...
  it('auto-rotates through articles', () => {
    ...
    expect(screen.getByText('This is the first featured article description')).toBeInTheDocument()

    // Advance timer by 1 second - should show second article
    act(() => {
      vi.advanceTimersByTime(1000)
    })
    expect(screen.getByText('This is the second featured article description')).toBeInTheDocument()

    // Advance timer by another second - should show third article (no description)
    act(() => {
      vi.advanceTimersByTime(1000)
    })
    expect(screen.queryByText('This is the second featured article description')).not.toBeInTheDocument()
