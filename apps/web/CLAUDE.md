...
- **A dynamic segment with `revalidate` needs `generateStaticParams` too.** Without it, `[param]` routes are server-rendered per request (`ƒ` in `next build`) and `revalidate` is inert config no request can honour (#2391). Export `generateStaticParams` returning `[]` — it enumerates nothing at build time (no PSD/BFF fetches, so the rate-limit rule is unaffected) but flips the segment to SSG + `dynamicParams`: unknown params render once on demand, then CDN-served on the declared interval. Converse: a segment dynamic for a structural reason (awaiting `searchParams`, as `/nieuws` does) must **not** export `revalidate` at all.
