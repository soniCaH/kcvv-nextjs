...
| `<HighlighterStroke>`                                    | SVG hand-drawn underline beneath italic emphasis, applied as a CSS `background-image` (data URL) on a single `<span>`. Three hand-drawn variants for a non-mechanical feel. **Phase 0: single-line only**; multi-line wrapping deferred to Phase 1+ (story file documents the limitation via a `MultiLineUnsupported` story flagged with `parameters: { vr: { disable: true } }`) | `variant: 'a' \| 'b' \| 'c'`, `color: 'jersey'`                                                                                      |
...
| `<ClippedCard>`      | Bordered "archival document" paper card with TL + BR L-shaped corner-clip accents (rendered internally). **No offset shadow, no rotation.** Distinct mood from `<TapedCard>` — used for forms / archival surfaces / official notices. The two card primitives must not be combined; their prop surfaces are deliberately non-overlapping             | `children`, `className?`, `as: 'div' \| 'article' \| 'section' \| 'figure'`                                                                                                                                                                     |
| `<StampBadge>`       | Content-bearing rotated badge with paper offset shadow. Pins to a `position: relative` parent (e.g. `<ClippedCard>`). Distinct from `<TapeStrip>` (graphical washi-tape) — `<StampBadge>` carries text. Border + shadow stay ink across all tones (chrome rule)                                                                                      | `children`, `rotation?: number` (default `2`), `position: 'top-right' \| 'top-left'`, `tone: 'jersey' \| 'ink' \| 'alert'`, `className?`                                                                                                        |

**Mood split — `<ClippedCard>` vs. `<TapedCard>`:** the two card primitives express different moods. `<TapedCard>` is for _loose paper on a desk_ (casual notices, taped-up artefacts, editorial figures) — rotated, shadowed, optionally taped. `<ClippedCard>` is for _clipped paper on a clipboard_ (archival, official, forms) — flat, bordered, with internal L-mark accents at TL/BR. The visual conflict (corner-clips on a rotated, shadowed, taped card) is made unrepresentable by the disjoint prop surfaces. A "don't" Storybook story (`UI/ClippedCard › DontMixWithTapedCard`) illustrates the boundary.
...
8. `<ArticleCredits>` — centered key/value credit panel at prose width, framed by 1px ink rules top + bottom. Rows: `Door` (author) · `Met` (subjects) · `Beeld` (photographer) · `Gepubliceerd` (date). Keys in mono caps, names in italic Freight Display 700. Rows with empty source fields are dropped. Cross-variant — renders whenever any credit field is populated. Replaces the retired `<InterviewCredits>` (per `docs/design/mockups/phase-5-article-detail/interview-locked.md` round 2 lock). Share buttons live in `<ArticleMetadata>` at the top of the article, not inside this block.
9. `<SiteFooter>`.
...
**Phase 6.A locked composition** (2026-05-21, after the 6.d0 → 6.d9 design drill series; lock docs in `docs/design/mockups/phase-6-player-profile/*-locked.md` are authoritative):

Stack:

1. `<SiteHeader>`.
2. `<MatchStrip>`.
3. `<PlayerHero>` — `<MonoLabelRow>` (team kicker) + `<NumberDisplay size="display-2xl" tone="jersey">#N</NumberDisplay>` + `<EditorialHeading>` with name rhythm (first name in Black upright + last name in italic display — per 6.d1 lock) + metadata row `position · birthDate` (age-graded per 6.d9; full DOB for adults, `<age> jaar · '<YY>` for minors) + right-column `<TapedFigure aspect="portrait-3-4">` rendering `player.psdImage` with newsprint filter + paper grain + `<TicketStub>` (team / season). When `psdImage` is missing, the frame falls back to the canonical `<PlayerFigure>` illustration (Phase 4.5 lock, refined at hero scale in queued 6.d2.a).
4. `<StripedSeam>`.
5. `<BioBlock>` — `<MonoLabelRow>* EVEN VOORSTELLEN</MonoLabelRow>` + `<EditorialHeading>` + paragraph from `player.bio` Portable Text + right-column `<PullQuote tone="jersey">` rendering the **first** `pullquote`-marked span from the bio (PT decorator locked at 6.d5).
6. `<QuotesBlock>` — `<EditorialHeading emphasis={{text:"woorden"}}>In zijn eigen woorden.</EditorialHeading>` + single full-width `<PullQuote tone="ink">` card rendering the **second** `pullquote`-marked span from `player.bio` (6.d8). Hosts the page's dark-band aesthetic (parked at 6.d4, homed here).
7. `<MatchStrip>` (mirrors top).
8. `<SiteFooter>`.

**Cross-age coverage** per 6.d9 + `[[project_player_profile_all_ages]]`: page renders for ALL ages (A-team through U6). BioBlock + QuotesBlock auto-hide when `player.bio` is empty (editorial policy: bios are not authored for minors). Hero meta row is age-graded — adults see full DOB, minors see `<age> jaar · '<YY>`.

**Schema deltas shipping with Phase 6.A:**

- Additive: `pullquote` Portable Text decorator on `player.bio` block marks
- Removals: drop `player.nationality`, `player.height`, `player.weight` (sync code + GROQ + migration; all editor-cost-free)

**Sections dropped** from the original canonical §5.3 spec (consult per-section lock docs for rationale):

- `<MonoLabel>NIEUW</MonoLabel>` hero badge — 6.d3
- `<StatsStrip>` band — 6.d4 (dark-band aesthetic relocated to QuotesBlock)
- `<CareerLogTable>` — 6.d6 (editorial cost without confident data)
- `<RecentMatchesGrid>` — 6.d7 (engineering cost + multi-team blocker)
- `<QuotesBlock>` cream half — 6.d8 (single ink card instead of pair)

**Phase 6 design-drill output:** `docs/design/mockups/phase-6-player-profile/` — per-element `*-locked.md` files. The original brief at `docs/design/phase-6-player-profile-brief.md` is preserved as historical record; the lock docs are authoritative for PRD writing and future builds.
...
- **Hero composition is sibling-per-surface, not a single variant union.** `<EditorialHero>` keeps its article-variant set (announcement / interview / event / transfer). Player profile uses `<PlayerHero>`; team detail uses `<TeamHero>`; match detail uses `<MatchHero>`. The original "one EditorialHero" thesis was revisited at Phase 6.A drill 6.d0 (2026-05-21) and rejected — heroes share sub-components (HeroCoverImage, Kicker, Byline) but not the top-level shell. See `docs/design/mockups/phase-6-player-profile/data-reality-locked.md`.
...
> **STATUS — ✅ SHIPPED (6.C, 2026-06-01).** `/ploegen` rebuilt as A+B paired mirrored flagships (`<TeamFlagship>`, A jersey-deep content-left/photo-right; B cream mirrored) + `<YouthDirectory>` grouped Bovenbouw/Middenbouw/Onderbouw. The uniform `<TapedCardGrid>` above was rejected. Shipped via #1946. Legacy `<TeamFeaturedCard>` / `<YouthTeamsDirectory>` retired in #1947. See `docs/prd/redesign-phase-6c-team.md` + `docs/design/mockups/phase-6-team/listing-locked.md`.
...
> **STATUS — ✅ SHIPPED (6.C, 2026-06-01).** `/ploegen/[slug]` rebuilt as a single-scroll + sticky-nav page: `<TeamHero>` → `<StandingsTable>` → `<TeamMatchesSection>` (+ `/wedstrijden` full agenda route) → `<SquadGrid>` → `<TeamStaff>` → `<TeamEditorial>` → global `<SponsorsBlock>`, every non-hero section auto-hiding on empty. **Dropped:** standalone `<StatsStrip>` and team-filtered `<SponsorsBlock>` (global block at the foot instead). Shipped via #1938 (tracer) + #1939–#1945. Legacy tabbed `<TeamDetail>` / `<TeamStandings>` / `<TeamSchedule>` retired in #1947. See `docs/prd/redesign-phase-6c-team.md` + `docs/design/mockups/phase-6-team/detail-ia-locked.md`.
...
| **5**  | Article detail (interviews first)                               | **Mockup per variant** — duo interview is mocked; matchverslag, column, transfer, jeugd, evenement, generic each need a mockup                                                                                                                                         | `<InterviewHero>`, `<QARow>`, `<QASection>`, `<ArticleCredits>`                                                                                                                                                                                                                        | All article variants. `docs/prd/article-detail-redesign.md` superseded                                                                                                                               | 2–3 weeks design + 3–4 weeks build |
| **6**  | Player profile + match detail + team detail + calendar + events | **Mockup per page type** — player profile is mocked and drilled (Phase 6.A LOCKED 2026-05-21 — see `docs/prd/redesign-phase-6a-player-profile.md`); match detail (3 states), team detail, kalender, events list + detail each need their own drill series in Phase 6.B | `<PlayerHero>` (6.A), reworked `<BioBlock>` + `<QuotesBlock>` (6.A); `<MatchHero>`, `<TeamHero>`, `<SquadGrid>`, `<MatchPreviewBody>`, `<EventCard>` (6.B). `<CareerLogTable>` + `<RecentMatchesGrid>` dropped at 6.d6 / 6.d7. `<StatsStrip>` stays in design system (6.B reuse only). | Player profile (6.A — shipped first); match detail, team detail, kalender, events (6.B — re-planned after 6.A retro)                                                                                 | 3–4 weeks design + 4–5 weeks build |
...
| **10** | Missed surfaces (finish the redesign)                           | **Mockup per surface** — generic page-hero + /club index + /club/[slug] + /club/contact + /staf detail each need a checkpoint; cookie banner a quick pass                                                                                                              | new generic editorial page-hero (replaces legacy `InteriorPageHero`)                                                                                                                                                                                                                   | Generic page-hero (#2120), /club index (#2121), /club/[slug] CMS (#2122), /club/contact (#2123), /staf/[slug] detail (#2124), cookie banner reskin (#2125). Discovered during #1531 spec 2026-06-14. | 1–2 weeks design + 2–3 weeks build |
...
**Completed phases:** Phase 0 (#1519), Phase 1, Phase 2, Phase 3, Phase 4 / 4.5 (homepage), Phase 5 (article detail — closed 2026-05-21 via #1860), Phase 6 (player / match / team detail — 6.A–6.C), Phase 7 (club / jeugd / teams / sponsors landings), Phase 8 (search / privacy / errors — closed 2026-06-14 via #2108), Phase 9 cleanup (#1531 — Phase B tokens via #2154, Phase A orphan deletes), and Phase 10 (missed surfaces, #2120–#2125). **The redesign series is complete — closed 2026-06-19 (decision-log entry 22).** Phase 10 was discovered during the #1531 spec (2026-06-14): the redesign was ~90% done and had skipped the generic page-hero, the `/club` index, `/club/[slug]` CMS pages, `/club/contact`, the `/staf/[slug]` detail, and the cookie banner (entry 19).
...
9. **Highlighter underline = SVG, not `text-decoration`.** Hand-drawn irregularity. **Phase 0 ships single-line only**; multi-line wrapping is deferred to Phase 1+. The CSS `background-image` data-URL approach used in Phase 0 lays one stroke across the highlighted span's bounding box and does NOT repeat per visual line when text wraps. The Phase 0 `<HighlighterStroke>` story file documents the limitation via a `MultiLineUnsupported` story flagged with `parameters: { vr: { disable: true } }` so the broken visual is not baselined.
...
19. **Phase 10 discovered — the redesign was ~90% done, not complete.** (2026-06-14) Spec'ing the Phase 9 cleanup (#1531) + its recolour spin-outs (#2116 font, #2117 colour) revealed that "remove all legacy tokens" was premised on a finished redesign — but a production-reachability + fate audit found a cluster of surfaces that were never migrated to retro-terrace and still hold the tokens. Resolution, decomposed: **(a) Deletes** → #1531 Phase A absorbs ~30 verified production-orphans (incl. the `_legacy/` trio, the whole `organigram/shared/` kit, `BrandedTabs`/`SocialLinks`/`ShieldFigure`/`PlayerFigure`) plus the 4 redundant per-route 404s (they fall through to the redesigned global `<ErrorState>`). **(b) Recolours** → #2116/#2117 rescoped to the surfaces staying structurally as-is (8 shipped-redesign-debt files + a few utility surfaces); font principle locked as Freight-for-debt-files / Montserrat-for-never-redesigned; colour mapping context-aware (bright `jersey` on dark, `jersey-deep` on light) with team-page status vocabulary. **(c) Phase 10 redesigns** → the genuinely-missed surfaces become #2120 (a new generic editorial page-hero replacing `InteriorPageHero` across `/kalender` + `/club` + `/club/[slug]` + `/scheurkalender`), #2121 (`/club` index — also kills the banned "Meer dan een voetbalclub" motto), #2122 (`/club/[slug]` CMS), #2123 (`/club/contact`), #2124 (`/staf/[slug]` detail), #2125 (cookie banner reskin). Decisions: **Montserrat is kept** as the body font; **Typekit is trimmed, not dropped** (Freight is on the same Adobe kit); `/tegenstander` is **kept** (owner tool) and recoloured. #1531's final token removal is blocked-by all of (b) + (c). Each Phase 10 page/composite surface still owes a `/design-an-interface` checkpoint per entry 15.

20. **Phase 9 cleanup complete — legacy tokens, Lucide, and the diagonal seam all retired (#2154).** (2026-06-18) The owner-reviewed migrate-vs-remove deep-dive spun out of #1531 Phase B (#2154) finished the redesign's final cleanup, decided hands-on against a live dev server + Storybook:
    - **Legacy tokens removed from `globals.css`:** all `--color-kcvv-*` (`:root` + `@theme` + ring + dark-blue), the `--color-green--*` alpha set, the legacy `--color-black`/`--color-gray--*`/`--color-green`/`--color-green-dark-hover` compat block, `--shadow-green`, and the title/alt fonts `--font-family-title` (Quasimoda) + `--font-family-alt` (Stenciletta). Kept: `--font-family-body` (Montserrat), `--font-family-mono` (IBM Plex), `--font-display`/`--font-display-big` (Freight), `gray-100`, table colours, foundation-gray. The dead `.featured-border`/`.after-border` utilities were deleted and the green tap-highlight moved to `color-mix(jersey)`.
    - **Global base rules rewritten** (entry 17's "legacy components keep Lucide until their own phase" now fully resolved): `body` → cream/ink; `h1–h6` → `--font-display`/ink and the `margin-bottom: 1em` footgun dropped; `main a`/`article a` → `jersey-deep` (the dead `.prose`/`.content`/`.content-link` rules removed).
    - **Lucide fully retired (Tier-2 of entry 17 completed end-to-end).** `<Icon>` + `src/lib/icons.ts` + the `BrandIcons` SVGs deleted; all ~12 importers (SiteHeader/SiteFooter chrome, MatchEvents/MatchLineup, ArticleBody, DownloadButton, EventFactOverview, ArticleMetadata, TransferFactOverview, SharePage, Input story/test) migrated to `@/lib/icons.redesign` Phosphor Fill wrappers; `lucide-react` dropped from `package.json`. New glyphs added incl. owner-picked **SoccerBall** (goal), **Swap** (substitution), **PaperPlaneTilt** (email field), FacebookLogo/InstagramLogo (footer social), ShareNetwork, ArrowSquareOut. `icons.redesign.ts` is now `"use client"` (Phosphor's module-level `createContext` breaks Server Components like `<SiteFooter>` otherwise).
    - **`SectionTransition` diagonal feature removed (the #1701 leftover).** `type:"diagonal"`/`"double-diagonal"` config + SVG-polygon rendering + `via`/`overlap`/`revealFrom`/`revealTo` + `DIAGONAL_HEIGHT`/`BG_COLOR` are gone; the component is striped-seam-only. `SectionStack` simplified to match. The `--footer-diagonal` token removed. `SectionBg` `kcvv-green-dark`→`jersey-deep` (1:1 `#008755`), `kcvv-black` dropped (homepage youth section migrated).
...
22. **Redesign complete — the editorial-redesign series is closed (#1531).** (2026-06-19) With the recolour spin-outs and Phase 10 redesigns merged, #1531's gated Phase A + B both shipped and the retro-terrace-fanzine system now covers every production surface:
    ...
    - **Recolours:** `font-title` (Quasimoda) consumers → Freight (#2116) and residual `--color-kcvv-*` consumers → retro-terrace tokens (#2117); `/share` Instagram match-share graphics moved onto the system (#2156).
    - **#1531 Phase A deletes:** ~30 production-orphan components removed (the `_legacy/` trio, `organigram/shared/`, `BrandedTabs`/`SocialLinks`/`ShieldFigure`/`PlayerFigure`, the orphan `article`/`responsibility`/`player` components, dead hooks/utils); the 4 redundant per-route 404s deleted (they fall through to the global `<ErrorState>`); `<SanityArticleBody>` deleted once #2122 + #2124 migrated its last consumers; `<NewsCard>` dead props (`eventDate`/`eventTime`/`countdown`/`isExternal`) removed; `apps/web/CLAUDE.md`'s `/club/organigram`→`/hulp` route drift reconciled.
    - **#1531 Phase B** (legacy tokens / Lucide / diagonal seam) shipped via #2154 (entry 20); the body-font swap to Freight Sans via #2174 (entry 21) was the final type decision; a Storybook ↔ live-component reconciliation + design-system convergence pass closed the last drift (#2155/#2176); the shared `<JerseyIllustration>` extraction (#2118, PRD §8) also landed.
    - **Status:** all phases 0–10 complete, no redesign follow-ups open. The brownfield mixed-state period is over — the site is fully on retro-terrace-fanzine.
