...
motion-reduce:transition-none` on every logo `<Image>`.
  - **Superseded for `/sponsors` itself** by [#2511](https://github.com/soniCaH/www.kcvvelewijt.be/issues/2511)/[#2655](https://github.com/soniCaH/www.kcvvelewijt.be/issues/2655): on this page every logo now renders in colour at rest, at every tier. This locked record is unchanged for the wall (`<SponsorsBlock>`, homepage + team pages), which the decision left untouched.
