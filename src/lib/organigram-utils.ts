/**
 * Organigram Utility Functions
 *
 * Helper functions for working with organigram data:
...
/**
 * Find a member in the organigram by their ID
 *
 ...
 *   view: 'chart'
 * });
 * // Returns: '/club/organigram?member=president&view=chart'
 * ```
 */
export function buildOrganigramUrl(
  basePath: string,
  options: {
...
/**
 * Parse URL parameters for organigram
 *
 ...
 * @example
 * ```ts
 * const params = parseOrganigramParams('?member=president&view=chart');
 * console.log(params.memberId); // 'president'
 * console.log(params.view); // 'chart'
 * ```
 */
export function parseOrganigramParams(searchParams: URLSearchParams | string): {
  memberId: string | null;
  view: string | null;
} {
  const params =
